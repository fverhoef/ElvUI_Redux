...
local function UpdateBorder(border, force)
    ...
    if not E.db[addonName].borders.enabled or not atlas or border.isHidden then
        border:Hide()
...
local function SetBorderScale(border, atlas)
    ...
    if not E.db[addonName].enabled or not atlas then
        return
    end
...
function Addon:CreateBorder(frame, configKey, anchor, layer)
    if not frame then
        return
    end
